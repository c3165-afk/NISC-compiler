...
> **2026-09-16 更新**：既定のコンパイル対象は32ビットintです。整数比較、`if/else`、`while`・`for`、ループ内を含む途中の`return`に対応しています。
> 採用仕様、引数・戻り値のSRAM配置、実行モデル、テスト方法は
> [int32到達点の仕様と検証](docs/int32-milestone.md)を参照してください。
> 下記のWSL手順以降にある第1章以降は引継時の説明です。旧経路は`profile="legacy"`で明示的に選択しますが、今回の動作保証対象ではありません。
...
整数比較・分岐・ループのテストは`tests/test_branches.py`にあり、同じコマンドで実行されます。
スコープの回帰テスト8件は`tests/test_scopes.py`にあります。
直線計算60プログラム、分岐30プログラム×5入力、途中のreturnを含む15プログラム×5入力、ループ12プログラム×5入力の比較も、この56件の中に含まれます。
A09の完了時刻検査とA21の回帰テストとして、既存の`tests/test_int32.py`に9件を追加しています。
各経路で演算完了・ブロック遷移・戻り値STORE・doneの順序を検査し、故意に壊した時刻や制御も拒否することを確認します。
検査の配置と判定基準は[完了時刻と回帰テスト](docs/int32-branches.md#a09の完了時刻検査とa21の回帰テスト)を参照してください。
比較条件の読み出しは既存の`mlir_to_cdfg/core.py`、比較コードと入力順序の変換は
既存の`code_gen/emitter.py`に集約し、int32経路と旧経路で共通利用しています。
共通化の検証範囲は[分岐拡張の修正記録](docs/int32-branches.md#比較処理の共通化)を参照してください。
...
[input/int32_branch.c](input/int32_branch.c)は、`a <= b`なら`a + 10`、
そうでなければ`b * 2`を返すサンプルです。

```bash
.compiler/bin/python -B tools/run_int32.py input/int32_branch.c 3 5
# return_value: 13（条件が真）
.compiler/bin/python -B tools/run_int32.py input/int32_branch.c 7 5
# return_value: 10（条件が偽）
.compiler/bin/python -B tools/run_int32.py input/int32_branch.c 5 5
# return_value: 15（等しい場合も <= は真）
```

分岐を含む場合、`done_state`は状態の番号であり、実行サイクル数ではありません。
選ばれなかった側の状態は通らず、選ばれた側から合流先へ進みます。
生成する`controls.json`は分岐先を含むv2形式です。
...
単項マイナス、6種類の整数比較、`if/else`、`while`・`for`（入れ子を含む）、途中・末尾の`return`です。
文としての`i++`・`++i`・`i--`・`--i`も扱います。条件式などの中で値として使う更新演算は対象外です。
すべての分岐経路でint値を返す必要があります。例えば
`int f(int a){if(a<0)return -a;return a+1;}`を扱えます。
`return`した経路では後続のC文を実行せず、選んだ戻り値を共通終了ブロックでSRAMへ書き込みます。
比較の結果はintの0/1、`if(x)`の条件は0が偽・非0が真です。
実際に実行する経路の全中間結果がint範囲内に収まる入力を使用します。
`break`・`continue`・`do while`・配列・浮動小数点・除算・論理演算`&& || !`などはエラーになります。
途中のreturnの変換と制限は[制御フローの修正記録](docs/int32-branches.md#途中のreturnと共通終了ブロック)を参照してください。
詳細は [採用仕様と修正判定基準](docs/int32-milestone.md) を参照してください。

#### 既存のforサンプルで反復を確認する

ファイルを追加せず、既存の`input/test_for.c`を使えます。0から`n-1`までを加算します。

```bash
.compiler/bin/python -B tools/run_int32.py input/test_for.c 0
# return_value: 0（反復なし）
.compiler/bin/python -B tools/run_int32.py input/test_for.c 5
# return_value: 10
```

`done_state`は終了状態の番号で、反復を含む実行サイクル数とは異なります。
実行モデルは既定で100,000サイクルを上限とし、終了しないループや上限を超える入力では
`Execution step limit exceeded`で停止します。この上限はRTLへは生成しません。
ループの制約と実装箇所は[ループ対応の修正記録](docs/int32-branches.md#a11とwhileforへの対応)を参照してください。
...
- **`experiment.py`との違い**：既存の`experiment.py`はfloat配列を使うbicgを対象としており、
...
分岐拡張の仕様・A02/A03への対応範囲は
[整数比較とif/elseの修正記録](docs/int32-branches.md)を参照してください。
同名変数の宣言前に行った代入の保持については
[スコープの修正記録](docs/int32-scopes.md)を参照してください。
